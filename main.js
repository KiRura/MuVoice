...
client.once("ready", async () => {
    setInterval(async () => {
        ...
        client.user.setActivity({ name: `${getVoiceConnections.length} / ${client.guilds.cache.size} servers・${client.users.cache.size} users・${res.time} ms` });
    }, 60000);

    const data = [
        { // join
            name: "join",
            description: "VCに参加, 読み上げ開始",
            options: [
                {
...
client.on("interactionCreate", async interaction => {
    try {
        ...
        if (interaction.commandName === "join") {
            ...
            if (!vc.joinable) return await interaction.reply({ content: "VCに参加できません。", ephemeral: true });
            if (!vc.speakable) return await interaction.reply({ content: "VCで音声を再生する権利がありません。", ephemeral: true });
            ...
            await interaction.reply("接続が完了しました。");
        };
        ...
        if (interaction.commandName === "leave") {
            ...
            if (!connection) return await interaction.reply({ content: "ごめん、VCにはいません。今、虚無にいます。", ephemeral: true });
            await interaction.reply("じゃあね！");
            connection.destroy();
            await wait(3);
            await interaction.deleteReply().catch(async e => { return; });
        };

        if (interaction.commandName === "setspeaker") {
            try {
                const speaker = interaction.options.get("speaker");
                try {
                    const user = fs.readFileSync(`users/${interaction.user.id}.txt`);
                    let jsonuser = JSON.parse(user);
                    jsonuser.speaker = speaker.value;
                    fs.writeFileSync(`users/${interaction.user.id}.txt`, Buffer.from(JSON.stringify(jsonuser)));
                } catch (error) {
                    fs.writeFileSync(`users/${interaction.user.id}.txt`, Buffer.from(JSON.stringify(
                        {
                ...
                const speakers = ["0", "1", "四国めたん", "ずんだもん", "4", "5", "6", "7", "春日部つむぎ", "波音リツ", "雨晴はう", "玄野武宏", "白上虎太郎", "青山龍星", "冥鳴ひまり"];
                await interaction.reply(`話者を${speakers[speaker.value]}に設定しました。`);
            } catch (error) {
                await interaction.reply({ content: `話者を変更できませんでした\n${error}`, ephemeral: true });
            };
        };
    } catch (error) {
        await interaction.reply({ content: `エラーにより実行できませんでした。\n${error}`, ephemeral: true });
        if (interaction.user.id !== "606093171151208448") await (await client.users.fetch("606093171151208448")).send(`${interaction.guild.name}の${interaction.user.tag}がデバッガーになりました。\n${error}`);
        console.log("スラコマのエラー");
        console.log(error);
    };
});
...
client.on("messageCreate", async message => {
    ...
    try {
        if (getVoiceConnection(message.guild.id)) {
            ...
            let voice;
            try {
                const user = fs.readFileSync(`users/${message.author.id}.txt`);
                voice = JSON.parse(user).speaker;
            } catch (error) {
                voice = default_voice;
                const json = {
                    speaker: default_voice
                };
                fs.writeFileSync(`users/${message.author.id}.txt`, Buffer.from(JSON.stringify(json)));
            };
            ...
            let msg = convertMessage(message.cleanContent);
            await generateAudio(msg, filepath, voice);
            await play(filepath);
        };
    } catch (error) {
        await message.author.send(`エラーにより処理できませんでした。\n${error}`).catch(async e => { return; });
        console.log("messageのエラー");
        console.log(error);
    };
});

client.login(process.env.DISCORD_TOKEN).catch(async e => console.log(e));